    ...
    const initMap = async () => {
      try {
        ...
        const map = new maplibregl.Map({
          ...
          style: `https://api.maptiler.com/maps/hybrid/style.json?key=${import.meta.env.VITE_MAPTILER_API_KEY || 'demo'}`,
        ...
        map.on('load', () => {
          if (terrainEnabled) {
            map.addSource('terrain', {
              type: 'raster-dem',
              url: `https://api.maptiler.com/tiles/terrain-rgb-v2/tiles.json?key=${import.meta.env.VITE_MAPTILER_API_KEY || 'demo'}`,
              tileSize: 256,
            })
      ...
      {isMapLoaded && (
        <div className="absolute top-4 right-4 flex flex-col gap-2">
          <Button
            ...
            title={terrainEnabled ? "Desativar terreno 3D" : "Ativar terreno 3D"}
            className="shadow-lg backdrop-blur-sm bg-background/90"
          >
            {terrainEnabled ? <Mountains size={20} weight="fill" /> : <Mountains size={20} />}
          </Button>
        ...
        <div className="absolute bottom-4 left-4 right-4 p-4 bg-background/90 backdrop-blur-sm rounded-lg shadow-lg space-y-3">
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              <label className="text-xs font-medium">Inclinação da Câmera</label>
              <span className="text-xs text-muted-foreground">{currentPitch}°</span>
            </div>
            <Slider
              value={[currentPitch]}
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-muted-foreground">{currentZoom.toFixed(1)}</span>
            </div>
            <Slider
              value={[currentZoom]}
