...
// Mock NASA POWER irradiation service
const mockNASASimpleIrradiation = async (lat: number, lon: number) => {
  ...
  return {
    lat,
    lon,
    irradiation_kwh_m2_day: baseIrradiation * latitudeFactor,
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Create cache key
    const cacheKey = `nasa:irradiation:${latNum}:${lonNum}`;
    const now = Date.now();
    ...
    if (cacheMap.has(cacheKey)) {
      ...
      if (now - cachedData.timestamp < CACHE_TTL) {
        console.log('NASA Irradiation Cache HIT:', cacheKey);
        return res.status(200).json(cachedData.data);
      }
    }

    console.log('NASA Irradiation API call:', { lat: latNum, lon: lonNum });

    // Get irradiation data
    const result = await mockNASASimpleIrradiation(latNum, lonNum);
    ...
    console.error('NASA irradiation endpoint error:', error);
