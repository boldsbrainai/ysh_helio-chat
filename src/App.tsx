...
import { Plus, PaperPlaneRight, Sparkle, List, Lightning, MagnifyingGlass, Share, Gear, ImageSquare, Code, Compass, FolderOpen, Sun } from "@phosphor-icons/react"
...
import { GPTsPage } from "@/components/pages/GPTsPage"
...
import { HomologationPage } from "@/components/pages/HomologationPage"
import { CreditAnalysisPage } from "@/components/pages/CreditAnalysisPage"
import { DashboardPage } from "@/components/pages/DashboardPage"
import { EarthObservationPage } from "@/components/pages/EarthObservationPage"
import { ChatKitDemoPage } from "@/components/pages/ChatKitDemoPage"
...
import { ReleaseNotesPage, TermsPage, BugReportPage, DownloadsPage, KeyboardShortcutsPage } from "@/components/pages/compliance"
import { RealtimeVoicePage } from "@/components/pages/RealtimeVoicePage"
import { TechnicalSheetsPage } from "@/components/pages/TechnicalSheetsPage"
import { LoginPage } from "@/components/pages/LoginPage"
import { CheckoutPage } from "@/components/pages/CheckoutPage"
import { ShadingAnalysisPage } from "@/components/pages/ShadingAnalysisPage"
import { TemporalAnalysisPage } from "@/components/pages/TemporalAnalysisPage"
import { SolarWorkflowPage } from "@/components/pages/SolarWorkflowPage"
import { PDFProposalGenerator } from "@/components/pages/PDFProposalPage"
...
export type RouteId = 'chat' | 'gallery' | 'codex' | 'gpts' | 'projects' | 'prompts' | 'equipment' | 'sizing' | 'homologation' | 'credit' | 'dashboard' | 'earth-observation' | 'chatkit-demo' | 'realtime-voice' | 'technical-sheets' | 'release-notes' | 'terms' | 'bug-report' | 'downloads' | 'shortcuts' | 'login' | 'checkout' | 'shading-analysis' | 'temporal-analysis' | 'solar-workflow' | 'pdf-proposal'
...
function App() {
  ...
  const [currentSessionId, setCurrentSessionId] = useKV<string | null>("current-session-id", null)
  ...
  useEffect(() => {
    ...
    const completeCallback = async () => {
      try {
        ...
        toast.success(`Bem-vindo, ${normalizedUser.name?.split(" ")[0] || "integrador"}!`)
        finalizeSuccess()
      } catch (callbackError) {
  ...
  const updateCurrentSession = (messages: ChatMessage[]) => {
    ...
    setChatSessions((current) => {
      ...
      if (messages.length > 0 && updatedSession.title === "Nova conversa") {
        ...
        if (firstUserMessage) {
          updatedSession.title = firstUserMessage.content.slice(0, 50) + (firstUserMessage.content.length > 50 ? "..." : "")
        }
      }
  ...
  const handleSend = async () => {
    ...
    const widgetMatch = Object.keys(widgetDemoMessages).find(key => lowerInput.includes(key))
...
Mensagem do usuário: ${trimmedInput}`
      ...
      const assistantMessage: ChatMessage = {
        ...
        content: response,
        timestamp: Date.now(),
        widget: widgetMatch ? widgetDemoMessages[widgetMatch as keyof typeof widgetDemoMessages] : undefined
      }
  ...
  const handleEditMessage = async (messageId: string, newContent: string) => {
    ...
    const updatedMessage = { ...chatMessages[messageIndex], content: newContent }
...
Mensagem do usuário: ${lastUserMessage.content}`
  ...
  const handleWidgetAction = (action: WidgetAction) => {
    ...
    toast.success(`Action: ${action.type}`, {
      description: action.payload ? JSON.stringify(action.payload, null, 2) : undefined
    })
  }
  ...
  }, [isPromptLibraryOpen, isSearchOpen, isShareOpen, isSettingsOpen, isSidebarOpen, isLoading])
    ...
    if (isProcessingFacebookOAuth) {
      return (
        <div className="min-h-screen flex flex-col items-center justify-center gap-4 bg-background text-center p-6">
          ...
          />
          <div>
            <p className="text-lg font-semibold">Validando login com o Facebook...</p>
  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden">
      ...
      {currentRoute === 'login' ? (
        <LoginPage onToggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} onLoginSuccess={handleLoginSuccess} />
      ) : currentRoute === 'checkout' ? (
        <CheckoutPage onToggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} />
      ) : currentRoute === 'equipment' ? (
        <EquipmentPage onToggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} />
      ) : currentRoute === 'technical-sheets' ? (
        <TechnicalSheetsPage onToggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} />
      ) : currentRoute === 'dashboard' ? (
        user ? <DashboardPage onToggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} /> : <EquipmentPage onToggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} />
      ) : currentRoute === 'sizing' ? (
        <SizingPage onToggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} />
      ) : currentRoute === 'earth-observation' ? (
        <EarthObservationPage onToggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} />
      ) : currentRoute === 'chatkit-demo' ? (
        <ChatKitDemoPage onToggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} />
      ) : currentRoute === 'realtime-voice' ? (
        <RealtimeVoicePage onToggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} />
      ) : currentRoute === 'homologation' ? (
        <HomologationPage onToggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} />
      ) : currentRoute === 'credit' ? (
        <CreditAnalysisPage onToggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} />
        ...
        <ProjectsPage onToggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} />
      ) : currentRoute === 'prompts' ? (
        <PromptsPage onToggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} />
      ) : currentRoute === 'release-notes' ? (
        <ReleaseNotesPage onToggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} />
      ) : currentRoute === 'terms' ? (
        <TermsPage onToggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} />
      ) : currentRoute === 'bug-report' ? (
        <BugReportPage onToggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} />
      ) : currentRoute === 'downloads' ? (
        <DownloadsPage onToggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} />
      ) : currentRoute === 'shortcuts' ? (
        <KeyboardShortcutsPage onToggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} />
      ) : currentRoute === 'shading-analysis' ? (
        <ShadingAnalysisPage onToggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} />
      ) : currentRoute === 'temporal-analysis' ? (
        <TemporalAnalysisPage onToggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} />
      ) : currentRoute === 'solar-workflow' ? (
        <SolarWorkflowPage onToggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} />
      ) : currentRoute === 'pdf-proposal' ? (
        <PDFProposalGenerator onToggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} />
      ) : (
      <div className="flex flex-col flex-1 h-screen overflow-hidden">
        <header className="flex items-center justify-between h-[60px] px-4 sm:px-6 border-b border-border/40 bg-card/50 backdrop-blur-xl shadow-sm flex-shrink-0" style={{ paddingLeft: 'max(1rem, env(safe-area-inset-left))', paddingRight: 'max(1.5rem, env(safe-area-inset-right))' }}>
          <div className="flex items-center gap-3 min-w-0 h-full">
            ...
            <motion.div 
              ...
              whileHover={prefersReducedMotion ? {} : { scale: 1.08, rotate: 8 }}
            ...
            <div className="min-w-0 flex flex-col justify-center -mt-px">
              ...
              <p className="text-[11px] text-muted-foreground font-medium hidden sm:block leading-[1.3] -mt-px">Sistema de gestão de energia solar</p>
            </div>
          </div>
          <div className="flex items-center gap-1 sm:gap-1.5 h-full">
            <motion.div whileHover={prefersReducedMotion ? {} : { scale: 1.05 }} whileTap={prefersReducedMotion ? {} : { scale: 0.95 }} style={{ willChange: 'transform' }} className="hidden md:block">
              <Button 
                variant="ghost" 
                size="icon" 
            ...
            <motion.div whileHover={prefersReducedMotion ? {} : { scale: 1.05 }} whileTap={prefersReducedMotion ? {} : { scale: 0.95 }} style={{ willChange: 'transform' }} className="hidden sm:block">
              <Button 
                variant="ghost" 
                size="icon" 
            ...
            <motion.div whileHover={prefersReducedMotion ? {} : { scale: 1.05 }} whileTap={prefersReducedMotion ? {} : { scale: 0.95 }} style={{ willChange: 'transform' }}>
              <Button variant="ghost" size="sm" onClick={handleNewChat} disabled={isLoading} className="hover:bg-accent/10 font-medium h-10 px-3">
                <Plus className="sm:mr-1.5" size={18} weight="bold" />
                <span className="hidden sm:inline text-[13px]">Novo Chat</span>
              </Button>
            </motion.div>
            <motion.div whileHover={prefersReducedMotion ? {} : { scale: 1.05 }} whileTap={prefersReducedMotion ? {} : { scale: 0.95 }} style={{ willChange: 'transform' }}>
              <Button
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-muted-foreground/80 leading-[1.5]">
                    Converse naturalmente sobre dimensionamento, equipamentos, financiamento, regulamentação e análise de sites com dados de alta precisão
                  </p>
                </div>
                ...
                >
                  <motion.div whileHover={prefersReducedMotion ? {} : { scale: 1.03, y: -2 }} whileTap={prefersReducedMotion ? {} : { scale: 0.98 }} style={{ willChange: 'transform' }}>
                    <Button
                      variant="outline"
                      ...
                      onClick={() => setInput("Analise minha fatura de energia e dimensione um sistema solar")}
                    >
                      <div className="flex items-center gap-2.5 w-full">
                        ...
                        <div className="flex-1 text-left min-w-0">
                          <span className="text-[14px] font-bold block leading-[1.3]">Dimensionar Sistema</span>
                          <span className="text-[11px] text-muted-foreground leading-[1.3] mt-0.5 block">A partir da sua fatura de energia</span>
                        </div>
                      </div>
                    </Button>
                  </motion.div>
                  <motion.div whileHover={prefersReducedMotion ? {} : { scale: 1.03, y: -2 }} whileTap={prefersReducedMotion ? {} : { scale: 0.98 }} style={{ willChange: 'transform' }}>
                    <Button
                      variant="outline"
                      className="h-auto py-4 px-4 flex flex-col items-start gap-2 w-full border-2 hover:border-[#FFD60A] hover:bg-[#FFD60A]/5 hover:shadow-lg transition-all group"
                      onClick={() => setInput("Selecione equipamentos para um projeto de 10kWp")}
                    >
                      <div className="flex items-center gap-2.5 w-full">
                        ...
                        <div className="flex-1 text-left min-w-0">
                          <span className="text-[14px] font-bold block leading-[1.3]">Escolher Equipamentos</span>
                          <span className="text-[11px] text-muted-foreground leading-[1.3] mt-0.5 block">Painéis, inversores e estruturas</span>
                        </div>
                      </div>
                    </Button>
                  </motion.div>
                  <motion.div whileHover={prefersReducedMotion ? {} : { scale: 1.03, y: -2 }} whileTap={prefersReducedMotion ? {} : { scale: 0.98 }} style={{ willChange: 'transform' }}>
                    <Button
                      variant="outline"
                      className="h-auto py-4 px-4 flex flex-col items-start gap-2 w-full border-2 hover:border-[#FFD60A] hover:bg-[#FFD60A]/5 hover:shadow-lg transition-all group"
                      onClick={() => setInput("Consulte irradiação solar em Belo Horizonte e estime a produção anual")}
                    >
                      <div className="flex items-center gap-2.5 w-full">
                        ...
                        <div className="flex-1 text-left min-w-0">
                          <span className="text-[14px] font-bold block leading-[1.3]">Consultar Irradiação</span>
                          <span className="text-[11px] text-muted-foreground leading-[1.3] mt-0.5 block">Dados de satélite (CAMS/NASA)</span>
                        </div>
                      </div>
                    </Button>
                  </motion.div>
                  <motion.div whileHover={prefersReducedMotion ? {} : { scale: 1.03, y: -2 }} whileTap={prefersReducedMotion ? {} : { scale: 0.98 }} style={{ willChange: 'transform' }}>
                    <Button
                      variant="outline"
                      className="h-auto py-4 px-4 flex flex-col items-start gap-2 w-full border-2 hover:border-[#FFD60A] hover:bg-[#FFD60A]/5 hover:shadow-lg transition-all group"
                      onClick={() => setInput("Simule financiamento para um projeto de R$ 45.000")}
                    >
                      <div className="flex items-center gap-2.5 w-full">
                        ...
                        <div className="flex-1 text-left min-w-0">
                          <span className="text-[14px] font-bold block leading-[1.3]">Simular Financiamento</span>
                          <span className="text-[11px] text-muted-foreground leading-[1.3] mt-0.5 block">Comparar bancos e condições</span>
                        </div>
                      </div>
                    </Button>
                  </motion.div>
      ...
      <div className="border-t border-border/40 bg-card/50 backdrop-blur-xl shadow-lg flex-shrink-0" style={{ paddingBottom: 'max(1rem, env(safe-area-inset-bottom))', paddingLeft: 'max(1rem, env(safe-area-inset-left))', paddingRight: 'max(1rem, env(safe-area-inset-right))' }}>
        <div className="max-w-4xl mx-auto px-4 sm:px-5 pt-4">
          ...
          >
            ...
            />
            <motion.div whileHover={prefersReducedMotion ? {} : { scale: 1.06 }} whileTap={prefersReducedMotion ? {} : { scale: 0.94 }} style={{ willChange: 'transform' }}>
              <Button
                type="submit"
          ...
          <p className="text-[11px] text-muted-foreground text-center mt-2 font-medium leading-[1.4]" id="input-hint">
